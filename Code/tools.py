...
def von_mises_unif(mu=0., kappa=1., n=100_000):
...
def von_mises_log(mu=0., kappa=1., n=100_000):
    """
    This function simulates a von Mises distribution using the
    rejection sampler based on a uniform distribution on [-pi, pi]
    as proposal distribution.
    ...
    # Compute the value for the rejection test
    sample1 = np.log(a1 * np.exp(-kappa) * C[C < Q1] + np.exp(a1 * -np.pi)) / a1
    val1 = np.exp(log_f(sample1) - a1 * sample1 - kappa)

    sample2 = np.log(a2 * np.exp(-b2) * (C[(Q1 <= C) & (C < Q2)] - Q1) + np.exp(a2 * -np.pi / 2)) / a2
    val2 = np.exp(log_f(sample2) - a2 * sample2 - b2)

    sample3 = np.log(a3 * np.exp(-b3) * (C[(Q2 <= C) & (C < Q3)] - Q2) + np.exp(a3 * z)) / a3
    val3 = np.exp(log_f(sample3) - a3 * sample3 - b3)

    sample4 = np.log(a4 * np.exp(-kappa) * (C[Q3 <= C] - Q3) + np.exp(a4 * np.pi / 2)) / a4
    val4 = np.exp(log_f(sample4) - a4 * sample4 - kappa)

    sample = np.hstack([sample1, sample2, sample3, sample4])

    # Acceptance step
    von_mises = sample[uniform(n) <= np.hstack([val1, val2, val3, val4])]
    ...
    while len(von_mises) < n:
        ...
        von_mises = np.hstack([von_mises, sample[uniform(n) <= np.hstack([val1, val2, val3, val4])]])
